...
test_stations <- "/Users/jeffkropelnicki/Desktop/Winfield files/weather requests/weather for john/Jeff_Only1recrodperAnswer.csv"

#Add the year you would like weather data for. 
year <- "2017"
...
#Makes new column with Lat. & Long. in radians for the Haversine math below. 
locations <- locations %>% 
...
# })
# 
# #makes a new colu,mn quadrant with that tells what quadrant the point belongs in.
# distances$Quadrant <- with(distances, Quadrant_1 + Quadrant_2 + Quadrant_3 + Quadrant_4)
# 
# #Returns 3 quadrants starting with the one labels nearness = 1 then moves to the nxt quadrant and so on.
# distances <- distances %>%
...
#prints the number of cores the computer has.
...
#Station_data_2015 <- full_join(distances, Station_data_2015, by = "ID")
# 
# #binds the rows for 2016 and 2017 together. 
#Station_data <- rbind(Station_data_2016, Station_data_2015)
...
files.to.delete <- dir("/Users/jeffkropelnicki/Desktop/Winfield files/Weather data and code/NOAA code/data_download/NOAA_weather_data",pattern=".op",recursive=T,full.names=T)
invisible(file.remove(files.to.delete))

write.csv(Station_data, "/Users/jeffkropelnicki/Desktop/test.csv")
